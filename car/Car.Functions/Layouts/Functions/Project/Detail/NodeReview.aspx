    ...
    <script>
        ...
        $(function () {
            ...
            $('#DataGridNodeReveiw').datagrid({
                ...
                columns: [[
                  { field: 'Project', title: '要素', width: "20%", formatter: function (val) { if (val) { return val.Label } else { return ""; } } },
                  { field: 'InCharge', title: '评价人员', width: "10%", formatter: function (val) { if (val) { return val.Label } else { return ""; } } },
                  {
                      field: 'Proportion', title: '占比%', width: "10%", formatter: function (val) {
                          return val * 100;
                      }
                  },
                  {
                      field: 'Score', title: '得分', width: "10%", editor: {
                  ...
                  {
                      field: 'Status', title: '状态', width: "10%", formatter: function (val) {
                          return '<img src="/_layouts/15/functions/images/' + val + '.png" style="height:22px;">';
                      }
                  },
                  { field: 'WeightedScore', title: '加权得分', width: "10%" },
                  { field: 'ProblemDescription', title: '问题描述', width: "20%", editor: { type: "textbox", options: { multiline: true } } },
                  {
                      field: 'Problems', title: '提示', width: "10%", formatter: function (value) {
            ...
            $('#DataGridUncomplete').datagrid({
                ...
                columns: [[
                    ...
                    {
                        field: 'ID', title: '<img src="/_layouts/15/functions/images/add-icno.png" alt="" style="width: 18px; height: 18px;" onClick="addUnComplete()">', width: "10%", formatter: function (val) {
                            return ' <img src="/_layouts/15/functions/images/close-icno.png" alt="" style="width: 20px; height: 20px; cursor: pointer;" onClick="DeleteUnComplete(' + val + ');" />';
                        }
                    }
                ]]
            });
                ...
                onClick: function () {
                    ...
                    $.WCFCallMethod("/_vti_bin/Car/PortalService.svc/").PostMethod("SaveTaskReviewTask", postData, function (responseData) {
                        if (responseData.Result) {
                            //刷新页面
                            $('#DataGridNodeReveiw').datagrid("load", { _t: new Date().getTime(), review: $("input[id$='HDReviewID']").val() });
                        }
                        else {
                            $('#DataGridNodeReveiw').datagrid("load", { _t: new Date().getTime(), review: $("input[id$='HDReviewID']").val() });
                        }
            ...
            SP.SOD.executeFunc('sp.ui.dialog.js', 'SP.UI.ModalDialog.showModalDialog', function () {
                var options = {
                    ...
                    dialogReturnValueCallback: function () {
                        $('#DataGridUncomplete').datagrid("load", {
                            review: $("input[id$='HDReviewID']").val(),
                            _t: new Date().getTime()
                        });
                    },
...
<asp:Content ID="Main" ContentPlaceHolderID="PlaceHolderMain" runat="server">
    ...
    <div class="panel panel-primary ">
        <div class="panel-heading ">
            Gate 1节点评审表单     <a id="btnEdit" >编辑</a>
            <a id="btnSave">保存</a>
        </div>
